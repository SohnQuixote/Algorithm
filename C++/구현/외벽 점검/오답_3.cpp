...
vector<vector<int>> repeated_perm;
void repeat_permutation(vector<int> weak, vector<int> perm, int depth)
{
    if (depth == perm.size())
    {
        vector<int> temp;
    ...
    {
        perm[depth] = weak[i];
        repeat_permutation(weak, perm, depth + 1);
    }
}
...
bool calc_clock(vector<int> comb, vector<int>dist,vector<int> temp_wall,vector<int> cc_wall,int M)
{
    ...
    if(accumulate(temp_wall.begin(),temp_wall.end() , 0) == temp_wall.size() || accumulate(cc_wall.begin(),cc_wall.end() , 0) == cc_wall.size())
    {
        ...
        for(int j=0;j<repeated_perm.size();j++)
        {
            for(int p=0;p<M;p++)
            {
                if(repeated_perm[j][p] != -1)
                {
                    int start = repeated_perm[j][p];
...
for(int M=1;M<=n_friends;M++)
{
  ...
  do{
    ...
    //comb가 1일때만 봄
      ...
      repeat_permutation(weak,weak_comb,0);
