      ...
      for(int k = possible_day;k<=N;k++)
        {
          if((sche[k].first+k) <=N+1)
          {
            ...
            //mov_sche[k].second = max({mov_sche[k].second, accu_pay+ sche[k].second,result});
            pair<int,int> tmp = minmax({mov_sche[k].second,(accu_pay+sche[k].second), result});
            mov_sche[k].second = tmp.second;
            result = mov_sche[k].second;
            //수행 가능한 스케줄일 경우
          }
          else{
