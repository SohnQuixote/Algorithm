...
int calc(vector<vector<pair<int,int>>> sea, int shark_dir,int shark_x,int shark_y, int shark_ate, pair<int,int> fishes[])
{
  //물고기
  int result = shark_ate;
  for(int i=1;i<17;i++)
    {
    ...
    for(int i=0;i<moving_shark.size();i++)
      {
        ...
        result = max(result,calc(sea,chged_shark_dir,moving_shark[i].first,moving_shark[i].second , shark_ate+feed, fishes));
...
int main() {
  ...
  sea[0][0].first = -1;
  //상어
  cout<<calc(sea,first_dir,0,0,first_food,fishes)<<endl;
}