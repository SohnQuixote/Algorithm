...
int bi_search_big(vector<string>words,int idx,int start, int end, char target)
{
   ...
   //첫번째 인덱스 우선이라서 글자마다 정렬해야함
    if((mid == words.size()-1 || target < words[mid+1][idx]) && target == words[mid][idx] )
        return mid;
    else if(target>= words[mid][idx])//원하는 글자가 앞에 있음
...
int bi_search_small(vector<string>words,int idx,int start, int end, char target)
{
    ...
    if(start>end)
        return IMPOSSIBLE;
    int mid = static_cast<int>((start+end)/2);
    if((mid == 0 || target> words[mid-1][idx]) && target == words[mid][idx] )
        return mid;
    else if(target<= words[mid][idx])//원하는 글자가 앞에 있음
...
pair<int,int> bi_search_cha(vector<string>words, string query,int idx,int start, int end)
{
    ...
    result.first = bi_search_small(words,idx,start,end,query[idx]);
    result.second = bi_search_big(words,idx,start,end,query[idx]);
...
int bi_search_whole(vector<string> words,string query)
{
    ...
    for(int i=0;i<q_length;i++)
    {
        ...
        range = bi_search_cha(tmp_words,query,i,range.first,range.second); //해당 범위 찾음
        tmp_words.clear();
