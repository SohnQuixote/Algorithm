...
int max_rect(int start, int end, int deg)
//start와 end 사이의 울타리 크기중 모두가 만족하는 최솟값 * end- start + 1
{
  ...
  answer = max(answer, (mid-start+1) *max_rect(start,mid-1,deg));
  //걸쳐 있을 경우?????
  ...
  while(mid - deg >= 0 || mid+deg <= N-1)
    {
      if((mid+deg) <= N-1)
        answer = max(answer,(deg+1) *         max_rect(mid,mid+deg,deg));
    if((mid-deg) >=0)
      answer = max(answer,(deg+1) * max_rect(mid-deg,mid,deg));
      deg++;
    }
  //중간값에서 하나씩?
  answer = max(answer, (end-mid) * max_rect(mid+1 ,end,deg));
  int mini = 987654321;
