        ...
        switch(b)
        {
            ...
            //삭제
                switch(a)
                {
                    ...
                    case 0:
                    //기둥 삭제
                        if(maps[x][y+1].first == 1)
                        {
                            //위에 기둥 존재
                            if(maps[x][y+1].second == 0 && maps[x-1][y+1].second == 0)
                                //위에 기둥 존재하는데 지탱할 보도 없음
                                break;
                        }
                        if(maps[x][y+1].second == 1)
                        {
                            //위에 보 좌측 꼭짓점 있는데 양옆에 보도 없고 지탱할 기둥도 없음
                            if(x<=n-1 && x>=1)
                            {
                                if((maps[x-1][y+1].second == 0 || maps[x+1][y+1].second == 0) && maps[x+1][y].first == 0)
                                    break;
                            }
                            else if(x ==0)
                            {
                                //맵 왼쪽 끝에 기둥 지우는데 위에 보 있고 우측에 기둥도 없음
                                if(maps[x+1][y].first == 0)
                                    break;
                            }
                        }
                    ...
                    {
                        //보 우측위에 기둥 존재
                        if(maps[x+1][y-1].first == 0)
                            break;
                        //밑에 기둥 없음
                    }
                    //좌측에 보 있을 때 보 밑에 기둥 없음
                    if(x>=1 && maps[x-1][y].second == 1)
                    {
                        if(maps[x-1][y-1].first == 0 && maps[x][y-1].first == 0)
                            break;
                    }
                    //우측에 보 있을 때 보 밑에 기둥 없음
                    if(x<=n-2 && maps[x+1][y].second == 1)
                    {
                        if(maps[x+1][y-1].first == 0 && maps[x+2][y-1].first ==0)
                            break;
                    }
                    break;
                    default:
