...
tuple<int,int,int,int,int> make_group(int x, int y, int g_num,vector<vector<tuple<int,int,int>>> &group)
{
  ...
  result = make_tuple(block_num,rainbow_num,cent_x,cent_y,g_num);
  return result;
}
//반시계 회전하기
//중력 작용시키기
void scoring_block(int x, int y)
{
  //dfs 한번 더 쓰는게 맞음
  int deleted = 1;
  queue<pair<int,int>>q;
  ...
  for(int i=0;i<N;i++)
    {
      for(int j=0;j<N;j++)
        {
          if(get<0>(group[i][j])  == 0 && grid[i][j] != -1 && grid[i][j] != -2)
          {
    ...
    for(int i=0;i<chk.size();i++)
      cout<<get<0>(chk[i])<<' '<<get<1>(chk[i])<<' '<<get<2>(chk[i])<<' '<<get<3>(chk[i])<<endl;
