...
vector<vector<vector<int>>> moving_history;
...
bool chk_history(vector<vector<int>> board, int N)
{
  bool result = true;
  for(int i=0;i<moving_history.size();i++)
    {
      if(!diff_board(moving_history[i],board,N))
      {
        //모든 로그 다 체크 해야함
        //하나라도 겹치면 안 가기
        result = false;
            ...
            for(int c = 0; c<N;c++)
              {
                if(board[r][c]>=2)
                {
                  ...
                  for(int k = N-1;k>=r;k--)
                    ...
                    //이동 안하면 그냥 놔둠 
                    for(int poss = final_row+1;poss<=N-1;poss++)
                      {
                        possible[poss][c] = false;
                      }
            ...
            for(int c = 0; c<N;c++)
              {
                if(board[r][c]>=2)
                {
                  ...
                  for(int k = 0;k<=r;k++)
                    ...
                    //이동 안하면 그냥 놔둠 
                    for(int poss = final_row-1;poss>= 0 ;poss--)
                      {
                        possible[poss][c] = false;
                      }
            ...
            for(int r = 0; r<N;r++)
              {
                if(board[r][c]>=2)
                {
                  ...
                  for(int k = N-1;k>=c;k--)
                    ...
                    //이동 열 다음 열부터 끝 열까지는 이동 불가능함
                    for(int poss = final_col+1;poss<=N-1;poss++)
                      {
                        possible[r][poss] = false;
                      }
                    //이동 안하면 그냥 놔둠
            ...
            for(int r = 0; r<N;r++)
              {
                if(board[r][c]>=2)
                {
                  ...
                  for(int k = 0;k<=c;k++)
                    ...
                    //이동 안하면 그냥 놔둠 
                  for(int poss = final_col-1;poss>= 0;poss--)
                      {
                        possible[r][poss] = false;
                      }
  ...
  for(int dir = 0;dir<4;dir++)
    {
      ...
      //print_board(board,N);
        /*moving_history.push_back(board);
        cout<<5-S<<"try::"<<endl;
        cout<<"moving_from::"<<endl;
        print_board(orig_board,N);
        cout<<"moving_to::"<<endl;
        print_board(board,N);
        //cout<<dir<<endl;*/
        //calc_max(board,N,S-1);
...
vector<vector<int>> board(N,vector<int>(N,0)); //숫자 담고 있음
  //moving_history.push_back(board);
//vector<vector<bool>>board_possible(20,vector<bool>(20,false)); //이동 했을 때 갈 수 있는지?
  for(int i=0;i<N;i++)
    ...
    //moving_history.push_back(board);
    //print_board(moving_history[0],N);
  ...
  //하 상 우 좌
  //0  1  2  3
    //board = moving_board(board,3,N);
    //print_board(board,N);
    /*board = moving_board(board,2,N);
    print_board(board,N);
  
    board = moving_board(board,1,N);
    print_board(board,N);
