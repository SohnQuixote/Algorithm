...
bool chk_finished(int tmp[10][10])
{
...
int  bridge_coord(int x, int y, int tmp[10][10], int d) {
  int e_x, e_y;
  int found = 0;
  int mov_x = x + dx[d];
  int mov_y = y + dy[d];
...
void chk_conn(int tmp[10][10], int sx, int sy, int bridges) {
  //재귀 호출
  if(chk_finished(tmp))
  {
            ...
            for(int d=0;d<4;d++)
              {
                int prev[10][10] = {0,};
                  for(int i=0;i<n;i++)
                    for(int j=0;j<m;j++)
                      prev[i][j] = tmp[i][j];
                int added = bridge_coord(x, y, tmp,d);
                if(added > 0)
                {
                  chk_conn(tmp,sx,sy+1,bridges+added);
                }
                  //print_island(tmp);
                  for(int i=0;i<n;i++)
                    for(int j=0;j<m;j++)
...
int main() {
  ...
  for (int i = 0; i < n; i++)
    for (int j = 0; j < m; j++)
      cin >> map[i][j];
  seperate_island(); //섬분리 완료
  // print_island(map);
  // cout<<bridge_coord(6, 2, map, 1)<<'\n';
  // print_island(map);
  // print_island();
  int tmp[10][10] = {0,};
  for(int i=0;i<n;i++)
