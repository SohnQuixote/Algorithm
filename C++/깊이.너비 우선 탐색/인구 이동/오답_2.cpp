  ...
  while(!q.empty())
    {
      ...
      q.pop();
      unions[union_number].push_back(make_tuple(map[x][y],x,y));
      ...
      for(int i=0;i<4;i++)
        {
          ...
          if(chk_boundary(mov_x,N) && chk_boundary(mov_y,N) && cities[x][y][mov_x][mov_y] == false) //바운더리에 있고 안가봄
          {
            ...
            {
              bool flag = false;
               for(int i=0;i<unions[union_number].size();i++)
                 {
                   int tmp_x = get<1>(unions[union_number][i]);
                   int tmp_y = get<2>(unions[union_number][i]);
                   if(tmp_x == x && tmp_y == y)   
                   {
                     flag = true;
                    break;
