...
int solution(vector<vector<int>> board) {
    ...
    visited[0][0][0] = true;
    //bool 배열 true인데 대체 왜가는건지 모르겠음 -> 큐에 넣을때 바꿔야지 중복해서 안넣음
    while(!q.empty())
    {
        int x = get<0>(q.front());
        int y = get<1>(q.front());
        ...
        switch(state)
        {
            case 0:
                ...
                for(int i=0;i<4;i++)
                {
                    mov_x = x+dx[i];
                    mov_y = y+dy[i];
                    if(calc_range(mov_x,N) && calc_range(mov_y,N) && calc_range(mov_y+1,N) && visited[mov_x][mov_y][state] == false &&board[mov_x][mov_y] != 1 && board[mov_x][mov_y+1] != 1)
                    {
            ...
            case 1:
                ...
                for(int i=0;i<4;i++)
                {
                    mov_x = x+dx[i];
                    mov_y = y+dy[i];
                    if(calc_range(mov_x,N) && calc_range(mov_x+1,N) && calc_range(mov_y,N) && visited[mov_x][mov_y][state] == false && board[mov_x][mov_y] != 1 & board[mov_x+1][mov_y] != 1)
                    {
                        visited[mov_x][mov_y][state] =true;
                        q.push(make_tuple(mov_x,mov_y,state,sec+1));   
